/// copy-paste of shuternay's unmerged fork https://github.com/shuternay/rust-wkb/blob/main/src/lib.rs
/// of https://github.com/amandasaurus/rust-wkb/
/// which fixes read/write of MultiPoint
use std::fmt::Debug;
use std::io;
use std::io::prelude::*;
...
fn read_point(mut wkb: impl Read) -> Result<Coord<f64>, WKBReadError> {
...
fn read_many_points<I: Read>(mut wkb: I) -> Result<Vec<Coord<f64>>, WKBReadError> {
    let num_points = read_u32(&mut wkb)? as usize;
    let mut res: Vec<Coord<f64>> = Vec::with_capacity(num_points);
...
pub fn geom_to_wkb<T: Into<f64> + Float + Debug>(
    geom: &Geometry<T>,
...
pub fn write_geom_to_wkb<W, T>(geom: &Geometry<T>, mut result: &mut W) -> Result<(), WKBWriteError>
where
    T: Into<f64> + Float + Debug,
    W: Write + ?Sized,
{
